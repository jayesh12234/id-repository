 ...
 * <p>
 * When {@code mosip.idrepo.policy.local-source=true}, policies and partner bioextractors are read
 * from classpath JSON under {@code pms_policy/} instead of remote PMS.
 * </p>
 */
@Component
public class PolicyUtil {
	...
	@Value("${mosip.idrepo.policy.local-extractors:classpath:pms_policy/partner-bioextractors.json}")
	private Resource localPolicyExtractors;

	private Map<String, JsonNode> partnerCredentialIndex = Map.of();
	private Map<String, JsonNode> policiesById = Map.of();
	private Map<String, PartnerExtractorResponse> partnerExtractorIndex = Map.of();
	...
	void loadLocalPolicyFiles() throws IOException {
		...
		if (localPolicyExtractors != null && localPolicyExtractors.exists()) {
			try (InputStream in = localPolicyExtractors.getInputStream()) {
				partnerExtractorIndex = mapper.readValue(in,
						new TypeReference<Map<String, PartnerExtractorResponse>>() {
						});
			}
			LOGGER.info(IdRepoSecurityManager.getUser(), LoggerFileConstant.SESSIONID.toString(),
					LoggerFileConstant.SESSIONID.toString(),
					"Loaded local partner bioextractors entries=" + partnerExtractorIndex.size());
		}
	}
	...
	private PartnerExtractorResponse getLocalPartnerExtractorFormat(String policyId, String subscriberId,
			String requestId) {
		PartnerExtractorResponse response = partnerExtractorIndex.get(subscriberId + "|" + policyId);
		if (response == null) {
			response = partnerExtractorIndex.get("default");
		}
		if (response == null) {
			response = new PartnerExtractorResponse();
			response.setExtractors(List.of());
		}
		LOGGER.info(IdRepoSecurityManager.getUser(), LoggerFileConstant.REQUEST_ID.toString(), requestId,
				"LOCAL bioextractors partnerId=" + subscriberId + ", policyId=" + policyId + ", extractors="
						+ (response.getExtractors() != null ? response.getExtractors().size() : 0));
		return response;
	}
	...
	public PartnerExtractorResponse getPartnerExtractorFormat(String policyId, String subscriberId, String requestId)
		...
		if (localPolicySource) {
			return getLocalPartnerExtractorFormat(policyId, subscriberId, requestId);
		}
		try {
			Map<String, String> pathsegments = new HashMap<>();
			pathsegments.put("partnerId", subscriberId);
